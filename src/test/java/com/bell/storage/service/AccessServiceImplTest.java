...
public class AccessServiceImplTest {
    ...
    @Before
    public void setUp() {
        service = new AccessServiceImpl(userDaoMock);
        ...
        when(userDaoMock.loadUserByUsername(USERNAME)).thenReturn(userMock);
        when(currentUserDtoMock.getUsername()).thenReturn(USERNAME);
        when(userDaoMock.getUserById(userId)).thenReturn(userMock);
    ...
    public void getAccessAndRequest() {
        ...
        Mockito.verify(userDaoMock, Mockito.times(1)).loadUserByUsername(USERNAME);
    }
    ...
    public void requestToVisibleAccess() {
        ...
        Mockito.verify(userDaoMock, Mockito.times(1)).loadUserByUsername(USERNAME);
        Mockito.verify(userDaoMock, Mockito.times(1)).getUserById(userId);
        Mockito.verify(userDaoMock, Mockito.times(1)).saveUser(userMock);
